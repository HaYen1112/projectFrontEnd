  ...
  constructor(private route: Router, private bodyCenter: BodyCenterComponent, private com: AppComponent) {
    ...
    if (typeof this.data !== 'undefined') {
      if ((this.data.length == 1)) {
        this.title = 'Kết quả tìm kiếm: ' + '"' + this.data[0] + '" không tìm thấy kết quả nào?';
              ...
              .subscribe((value: any) => {
                for (let item of value['items']) {
                  ...
                  this.dataLinkSearch.filter((res: any) => {
                    if (res['title'].toLocaleLowerCase().match(this.data[0].toLocaleLowerCase()) != null) {
                      this.dataItem.push({ image: res.image, title: res.title, time: res.time, link: res.link });
                      this.title = 'Kết quả tìm kiếm: ' + '"' + this.data[0] + '"';
                    }
                  });
                  this.dataLinkSearch = [];
                }
